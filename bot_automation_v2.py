...
def get_next_unpublished(posts: list, key: str = "published") -> dict | None:
    """Return the first post where key is not True."""
    for p in posts:
        if not p.get(key, False):
            return p
    return None
...
async def post_to_fb(fb_poster, post, msg_to_edit=None) -> dict:
    """Post to Facebook using the original bilingual post, return result."""
    if not fb_poster or not fb_poster.enabled:
        return {"success": False, "error": "Facebook not configured"}
    if post.get("published_fb"):
        return {"success": False, "error": "Already published on Facebook"}
    text = build_bilingual(post)
    media_file = post.get("post_media", "")
    media_path = MEDIA_DIR / media_file if media_file else None
    return await fb_poster.post_with_photo(text, media_path)
...
async def cmd_fb_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total = len(posts)
    published = sum(1 for p in posts if p.get("published_fb"))
    remaining = total - published
    next_fb = get_next_unpublished(posts, "published_fb")
    next_info = ""
    if next_fb:
...
async def cmd_fb_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/fb_now — Post the next unpublished post to Facebook"""
    fb_poster = context.application.bot_data.get("fb_poster")
    posts = context.application.bot_data.get("posts", [])
    if not fb_poster or not fb_poster.enabled:
    ...
    msg = await update.message.reply_text(
        f"📘 Posting Day {post['day']}, Post #{post['post_number']} to Facebook..."
    )
...
async def main():
    ...
    logger.info("🤖 Starting bot... /status, /post_now, /skip, /stats, /fb_status, /fbstats, /fb_now, /fb_skip")
